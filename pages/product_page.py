...
class ProductPage(BasePage):
    ...
    def prepare_expected_result(self, field_name=""):
        if field_name == "name": expected_result = self.browser.find_element(
            *ProductPageLocators.EXPECTED_NAME_OF_BOOK).text
        if field_name == "price": expected_result = self.browser.find_element(
            *ProductPageLocators.EXPECTED_PRICE_OF_BOOK).text
    ...
    def should_be_message_book(self):
        assert self.is_element_present(
            *ProductPageLocators.MESSAGE_BOOK_IN_BASKET), 'Message about adding book to basket is not presented'
    ...
    def should_be_message_price(self):
        assert self.is_element_present(
            *ProductPageLocators.MESSAGE_PRICE_OF_BASKET), 'Message about price in basket is not presented'
